...
import { categories, routines as offlineRoutines } from "./src/data/routines";
import { fetchPublishedRoutines } from "./src/lib/supabase";
...
function AppContent() {
  ...
  const [catalog, setCatalog] = useState<Routine[]>(offlineRoutines);
...
function TopBar({
  ...
  return (
    <View style={styles.topBar}>
      ...
      <Pressable
        onPress={onSaved}
        style={[styles.savedButton, activeView === "saved" && styles.savedButtonActive]}
        hitSlop={8}
        accessibilityRole="button"
        accessibilityLabel={`Open saved guides${savedCount ? `, ${savedCount} saved` : ""}`}
        accessibilityState={{ selected: activeView === "saved" }}
      >
        <Ionicons
          name={activeView === "saved" ? "bookmark" : "bookmark-outline"}
          size={17}
          color={COLORS.ink}
        />
  ...
  return (
    ...
    >
      <View style={styles.hero}>
        ...
        <Text style={styles.heroTitle}>What are you{"\n"}doing right now?</Text>
        <Text style={styles.heroBody}>
          Find out what matters, what you can skip, and what to use instead.
        </Text>
      </View>
      ...
      <View style={styles.searchWrap}>
        ...
        <TextInput
          ...
          placeholder="Try “deodorant” or “laundry”"
      ...
      <View style={styles.listHeader}>
        <Text style={styles.listTitle}>
          {query ? "MATCHING ROUTINES" : category === "All" ? "START HERE" : category.toUpperCase()}
        </Text>
        <View style={styles.listMeta}>
          {isLoadingLive ? <ActivityIndicator size="small" color={COLORS.muted} /> : null}
          <Text style={styles.resultCount}>
            {routines.length} {routines.length === 1 ? "GUIDE" : "GUIDES"}
          </Text>
        </View>
      </View>
      ...
      {!routines.length ? (
        <View style={styles.emptySearch}>
          ...
          <Text style={styles.emptyBody}>
            Try a broader search. New routines can be published without updating the app.
          </Text>
        </View>
      ) : null}
...
function RoutineCard({
  ...
  return (
    <Pressable
      ...
      accessibilityRole="button"
      accessibilityLabel={`Open guide: ${routine.title}. ${routine.prompt}`}
    >
      <View style={styles.cardNumberWrap}>
        <Text style={styles.cardNumber}>{String(index).padStart(2, "0")}</Text>
      </View>
      <View style={styles.cardMain}>
        <Text style={styles.cardCategory}>{routine.category.toUpperCase()}</Text>
        <Text style={styles.cardTitle}>{routine.title}</Text>
        <Text style={styles.cardPrompt}>{routine.prompt}</Text>
      </View>
      ...
      {routines.length ? (
        ...
        <View style={styles.savedEmpty}>
          <View style={styles.savedEmptyIcon}>
            <Ionicons name="bookmark-outline" size={30} color={COLORS.ink} />
          </View>
          <Text style={styles.emptyTitle}>No saved guides.</Text>
          <Text style={styles.emptyBody}>
            Bookmark routines you want to find quickly.
          </Text>
          <Pressable
            onPress={onExplore}
          ...
          >
            <Text style={styles.primaryButtonText}>EXPLORE ROUTINES</Text>
...
function RoutineDetail({
  ...
  const toneLabel =
    routine.answerTone === "yes"
      ? "YES, YOU CAN USE LESS"
      : routine.answerTone === "no"
        ? "DON’T SKIP THIS"
        : "IT DEPENDS";
    ...
    <View style={styles.detailShell}>
      <View style={styles.detailTopBar}>
        ...
        <Pressable
          ...
          hitSlop={8}
          accessibilityRole="button"
          accessibilityLabel={isSaved ? "Remove from saved guides" : "Save this guide"}
      ...
      >
        <View style={styles.detailHero}>
          <Text style={styles.detailCategory}>{routine.category.toUpperCase()}</Text>
          <Text style={styles.detailTitle}>{routine.title}</Text>
          <Text style={styles.detailPrompt}>{routine.prompt}</Text>
        </View>

        <View style={styles.answerCard}>
          <View style={styles.answerTopRow}>
            <Text style={styles.answerLabel}>{toneLabel}</Text>
            <Text style={styles.answerIcon}>{routine.icon}</Text>
          </View>
          <Text style={styles.answerText}>{routine.answer}</Text>
          <Text style={styles.answerSummary}>{routine.summary}</Text>
        </View>

        <DetailSection number="01" title="WHAT ACTUALLY MATTERS">
          <View style={styles.stepList}>
            {routine.essentials.map((step, index) => (
        ...
        <DetailSection number="02" title="CAN I SKIP IT?">
          <View style={styles.skipCard}>
            <Ionicons name="remove-circle-outline" size={24} color={COLORS.ink} />
            <Text style={styles.skipText}>{routine.skipNote}</Text>
          </View>
        </DetailSection>

        <DetailSection number="03" title="WHAT TO RECONSIDER">
          <View style={styles.avoidList}>
            {routine.avoid.map((item) => (
        ...
        <DetailSection number="04" title="DO THIS INSTEAD">
          <View style={styles.optionList}>
            {routine.options.map((option) => (
        ...
        <View style={styles.evidenceCard}>
          <View style={styles.evidenceHeader}>
            <View>
              ...
              <Text style={styles.evidenceStrength}>{routine.evidence}</Text>
            </View>
            <View style={styles.evidenceBars}>
              {[0, 1, 2].map((bar) => {
                const active =
                  routine.evidence === "Strong" ||
                  (routine.evidence === "Moderate" && bar < 2) ||
                  (routine.evidence === "Limited" && bar < 1);
                return (
                  <View
                    key={bar}
                    style={[styles.evidenceBar, active && styles.evidenceBarActive]}
                  />
                );
              })}
            </View>
          </View>
          <Text style={styles.evidenceNote}>{routine.evidenceNote}</Text>
          <Text style={styles.updated}>REVIEWED {routine.updatedAt.toUpperCase()}</Text>
        </View>

        <Text style={styles.disclaimer}>
          Educational information only—not medical or dental advice. Needs differ by
          person, condition, age, and exposure.
        </Text>
      </ScrollView>
    </View>
  );
...
const styles = StyleSheet.create({
  ...
  heroTitle: {
    ...
    lineHeight: Platform.OS === "web" ? 57 : 49,
    fontWeight: "900",
    letterSpacing: -2.4,
    maxWidth: 650
  },
  ...
  detailTitle: {
    color: COLORS.ink,
    fontSize: Platform.OS === "web" ? 55 : 46,
    lineHeight: Platform.OS === "web" ? 57 : 48,
    fontWeight: "900",
    letterSpacing: -2.2
  },
  detailPrompt: {
